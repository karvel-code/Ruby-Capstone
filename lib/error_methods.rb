      ...
      if x[2] != 'end'
        ...
        (h..x[2]).each do |n|
          if code[b].include?('elsif') || code[n].include?('else') || code[n].include?('end')
            @error << "line #{n + 1} is not properly indented::#{code[n]}".yellow unless code[n].index(/\S/) == x[0] - 2
          else
            @error << "line #{n + 1} is not properly indented::#{code[n]}".yellow unless code[n].index(/\S/) == x[0]
          end
        end
      else
        k = x[1] - 1
